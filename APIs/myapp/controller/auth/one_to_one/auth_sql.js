    ...
    try{
        ...
        try{
            ...
            if(results.affectedRows){
                const token = jwt.sign({ auth_id: results.insertId,email_id:email_id }, secret_key, { expiresIn: '1h' });
...
module.exports.welcome = async(req,res) =>{
    try{
        ...
        try{            
            const [results,feilds] = await sql_connection.query("select u.user_id,u.first_name,u.last_name,u.email_id from users u where u.email_id = ?",[email_id]);
            if(results.length){
                ...
                try{
                    const [auth_results,auth_feilds] = await sql_connection.query("select * from auths a where a.email_id = ?",[email_id]);
                    if(auth_results.length){
                        display.end_result(res,200,{"name":auth_results[0].name,"message":auth_results[0].name+' Please fill up other details'});  
                        return
                    }
                    else{
                        display.end_result(res,404,{"message":'user is not found'});  
