...
const upload_dir =
  "/node_js/Novastrid-NodeJS-sandbox/APIs/myapp/excel-download/users.xlsx";
    ...
    try {
      ...
      try {
        ...
        if (response.status === 200) {
          const workbook = xlsx.read(upload_dir);
          ...
          if (workbook.SheetNames.includes("users")) {
            ...
            const sql_user_ids = response.data.map(
              (user_item) => user_item.user_id
            );
            const excel_user_ids = data.map((user_item) => user_item.user_id);

            new_ids.concat(
              sql_user_ids.filter((value) => !excel_user_ids.includes(value))
            );

            need_to_add.concat(
              data.map((user_item) => new_ids.includes(user_item.user_id))
            );
          ...
          xlsx.writeFile(workbook, upload_dir);

          console.log(`updated successfully ${upload_dir}`);
        }
      } catch (error) {
